...
def view_details(s):
	if(s!=rras_code):
		...
		label=Label(root4,text="RESTAURANT REVIEW ANALYSIS SYSTEM",bd=2,font=('Arial',47,'bold','underline'),padx=80,pady=20)
		sug1=Label(root4,text="View the data from your database.",padx=40,pady=20)
		acc_btn=Button(root4,text="View Data",font=('Arial',20),padx=80,pady=20,command=access_data)
		sug2=Label(root4,text="Clear specific item data.",padx=80,pady=20)
		itemclr_btn=Button(root4,text="Clear Item Data",font=('Arial',20),padx=100,pady=20,command=clr_itemdata)
		sug3=Label(root4,text="Clear all item data.",padx=80,pady=20)
		...
		exit_btn.grid(row=9,column=0,pady=20,sticky=S)

			
def take_review():
	root2=Toplevel()
	root2.title(main+"give review")
	label=Label(root2,text="RESTAURANT REVIEW ANALYSIS SYSTEM",bd=2,font=('Arial',47,'bold','underline'),padx=80,pady=20)
	req1=Label(root2,text="Select the item(s) you have ordered.",padx=80,pady=20)
	'''foods=["Idly","Dosa","Vada","Roti","Meals","Veg Biryani","Egg Biryani","Chicken Biryani","Mutton Biryani","Ice Cream","Noodles","Manchooriya","Orange juice","Apple Juice","Pineapple juice","Banana juice"]
	for i in foods:
		food_rev[i]=[]
		food_perc[i]=[0.0,0.0]'''
	...
	exit_btn=Button(root2,text="Exit",command=root2.destroy)
	root2.state('zoomed')
	...
	exit_btn.grid(row=13,column=0,pady=20,sticky=S,columnspan=4)
...
def login():
	...
	label=Label(root3,text="RESTAURANT REVIEW ANALYSIS SYSTEM",bd=2,font=('Arial',47,'bold','underline'),padx=80,pady=20)
	label2=Label(root3,text="PLEASE ENTER YOUR LOGIN PASSWORD",bd=1,font=('Helvetica',30,'bold'))
	label3=Label(root3,text="ENTER PASSWORD :",bd=1,font=('Helvetica',20,'bold'),pady=20)
	global ent
	ent=Entry(root3,show="*",width=50,borderwidth=2)
	global show_password_button
	show_password_button = Checkbutton(root3, text="show",fg='red',font=('Helvetica',14), command=show_and_hide)
	show_and_hide()
	submit_code=Button(root3,text="Submit",font=('Arial',20),padx=80,pady=20,command=lambda : [view_details(ent.get()),root3.destroy()])
	exit_btn=Button(root3,text="Exit",command=root3.destroy)

	root3.state('zoomed')
	label.grid(row=0,column=0,columnspan=3)
	label2.grid(row=1,column=0,sticky=W+E,columnspan=3)
	label3.grid(row=3,column=0,columnspan=2,padx=20)
	ent.grid(row=3,column=1,columnspan=2,padx=0,pady=0)
	submit_code.grid(row=4,column=1,columnspan=1)
	show_password_button.grid(row=3, column=2,columnspan=2)
	exit_btn.grid(row=13,column=0,pady=20,sticky=S,columnspan=4)


#init_data()
label=Label(root1,text="RESTAURANT REVIEW ANALYSIS SYSTEM",bd=2,font=('Arial',47,'bold','underline'),padx=80,pady=20)
ques=Label(root1,text="Are you a Customer or Owner ?",pady=30)
cust=Button(root1,text="Customer",font=('Arial',20),padx=80,pady=30,command=take_review)
owner=Button(root1,text="Owner",font=('Arial',20),padx=100,pady=30,command=login)
